...
class TravelPlanner:
    ...
    def __init__(self, mode: str = "driving"):
        ...
        # 缓存机制：点对缓存，减少重复计算
        self.cache_file = f"travel_time_cache_{self.mode}.json"
    ...
    def plot_route_on_map(self, path: list, map_filename: str = "route_map.html"):
        ...
        # 绘制路线
        folium.PolyLine(coords_latlng, color="green", weight=5, opacity=0.7).add_to(m)

        # 保存为HTML
        m.save(map_filename)
        print(f"已生成路线地图: {map_filename}")
