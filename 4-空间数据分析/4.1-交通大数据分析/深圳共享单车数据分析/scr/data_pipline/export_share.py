...
def _ensure_dirs(base: Path, coord_sets: List[str] = ["raw", "wgs84"]) -> dict[str, dict[str, Path]]:
    """确保导出目录存在，支持多套坐标系"""
    dirs = {}
...
def _build_day_query_v2(conn, table: str, coord_set: str, day: date):
    ...
    if coord_set == "raw":
        ...
        if not has_start_raw and not has_start:
            raise RuntimeError(f"表 {table} 缺少原始坐标列（start_geom_raw/start_geom）")
        
        start_geom_expr = "start_geom_raw" if has_start_raw else "start_geom"
        end_geom_expr = "end_geom_raw" if has_end_raw else ("end_geom" if has_end else None)
        ...
        if not has_start_wgs84 and not has_start:
            raise RuntimeError(f"表 {table} 缺少WGS84坐标列（start_geom_wgs84/start_geom）")
        
        start_geom_expr = "start_geom_wgs84" if has_start_wgs84 else "start_geom"
        end_geom_expr = "end_geom_wgs84" if has_end_wgs84 else ("end_geom" if has_end else None)
...
def export_day(
    ...
    with psycopg.connect(conn_str) as conn:
        for coord_set in coord_sets:
            ...
            try:
                query = _build_day_query_v2(conn, table, coord_set, day)
                ...
                with conn.cursor(row_factory=dict_row) as cur:
                    ...
                    if "csv" in formats:
                        try:
                            csv_file = dirs[coord_set]["csv"] / f"bike_data_{day_str}.csv"
                            csv_zip_file = dirs[coord_set]["csv"] / f"bike_data_{day_str}.zip"
                            ...
                            stats["coord_sets"][coord_set]["formats"].append(f"CSV: {csv_zip_file}")
                    ...
                    if "geojson" in formats:
                        try:
                            geojson_file = dirs[coord_set]["geojson"] / f"bike_data_{day_str}.geojson"
                            geojson_zip_file = dirs[coord_set]["geojson"] / f"bike_data_{day_str}.zip"
                            ...
                            for row in all_rows:
                                # 跳过没有坐标的记录
                                if row.get(start_lng_field) is None or row.get(start_lat_field) is None:
                                    continue
                                ...
                                if (row.get(end_lng_field) is not None and 
                                    row.get(end_lat_field) is not None):
                                    end_feature = {
                                        "type": "Feature", 
                                        "geometry": {
                                            "type": "Point",
                            ...
                            stats["coord_sets"][coord_set]["formats"].append(f"GeoJSON: {geojson_zip_file}")
                            logger.debug(f"{coord_set} GeoJSON导出完成: {geojson_zip_file}")
                            
                        except Exception as e:
                            logger.error(f"{coord_set} GeoJSON导出失败: {e}")
                    
                    stats["coord_sets"][coord_set]["exported"] = len(all_rows)
                    logger.info(f"{day_str} {coord_set} 导出完成，共 {len(all_rows)} 条记录")
...
def export_date_range_v2(
    ...
    logger.info(f"开始导出 {len(dates)} 天的数据，坐标系: {coord_sets}，格式: {formats}")
            ...
            try:
                ...
                for coord_set in coord_sets:
                    if coord_set in stats["coord_sets"]:
                        total_stats["coord_sets"][coord_set]["total_records"] += stats["coord_sets"][coord_set]["exported"]
                        
            except Exception as e:
                logger.error(f"导出 {day} 失败: {e}")
                total_stats["failed_days"].append(str(day))
    
    logger.success(f"导出完成！成功: {total_stats['successful_days']}/{total_stats['total_days']} 天")
    for coord_set in coord_sets:
...
if __name__ == "__main__":
    ...
    ap.add_argument("--table", default="shenzhen_rides_v2", help="表名")
