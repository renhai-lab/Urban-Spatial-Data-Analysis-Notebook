        ...
        async with aconn.cursor() as acur:
            ...
            if profile.enable_timescale:
                try:
                    ...
                    logger.warning(f"TimescaleDB 扩展创建失败，将使用普通 PostgreSQL 表：{e}")
            ...
            if profile.enable_timescale:
                try:
                    ...
                    if timescale_sql.strip():
                        ...
                        logger.info(f"TimescaleDB 分区已设置：{profile.partition_interval} 间隔")
                except Exception as e:
                    logger.warning(f"TimescaleDB 分区设置失败：{e}")
